...
const TechnicalAidsPage = () => {
    ...
    setFilteredAids(
      aids.filter(aid => {
        const matchesSearch = (
          ...
          aid.school_year.includes(lowerSearchText)
        );
        const matchesYear = selectedSchoolYear === 'all' || aid.school_year === selectedSchoolYear;
  ...
  const columns = [
    ...
    },
    {
        ...
        sorter: (a, b) => `${a.student_name} ${a.student_surname}`.localeCompare(`${b.student_name} ${b.student_surname}`),
    ...
    },
    {
        ...
        sorter: (a, b) => dayjs(a.lending_date).unix() - dayjs(b.lending_date).unix(),
    },
    {
        ...
        sorter: (a, b) => dayjs(a.return_date || 0).unix() - dayjs(b.return_date || 0).unix(),
    },
    {
        title: 'Curso Escolar',
  ...
  return (
    <div>
      <Space style={{ marginBottom: 16, width: '100%', justifyContent: 'space-between' }}>
        <Space wrap> { /* Usar Space wrap para mejor layout en pantallas pequeñas */}
            <Input
      ...
      <Modal
        title={modalMode === 'add' ? "Registrar Nueva Ayuda Técnica" : "Editar Ayuda Técnica"}
        open={isModalVisible}
            ...
            >
              {students.map(student => (
                <Option key={student.id} value={student.id}>
                  {`${student.name} ${student.surname} ${student.second_surname || ''} (${student.student_number})`}
                </Option>
              ))}
            </Select>
          </Form.Item>
